-- ============================================================================
-- CRÉATION DE LA TABLE vocabulary_chunks_embeddings (Version corrigée)
-- ============================================================================
-- Ce script détecte automatiquement le type de vocabulary.id et crée la table
-- avec le bon type pour la clé étrangère
-- ============================================================================
...
-- ============================================================================
-- Détection du type et création de la table
-- ============================================================================
DO $$
DECLARE
  vocab_id_type TEXT;
BEGIN
  ...
  IF vocab_id_type IS NULL THEN
    RAISE EXCEPTION 'Table vocabulary n''existe pas. Créez d''abord la table vocabulary.';
  END IF;
  ...
  ELSE
    RAISE EXCEPTION 'Type inattendu pour vocabulary.id: %. Types supportés: uuid, bigint', vocab_id_type;
  END IF;
  
  RAISE NOTICE '✅ Table vocabulary_chunks_embeddings créée avec vocabulary_id de type %', vocab_id_type;
  
  -- Créer les index
  EXECUTE 'CREATE INDEX IF NOT EXISTS idx_vocab_chunks_vocabulary_id ON vocabulary_chunks_embeddings(vocabulary_id)';
  EXECUTE 'CREATE INDEX IF NOT EXISTS idx_vocab_chunks_user_id ON vocabulary_chunks_embeddings(user_id)';
  EXECUTE 'CREATE INDEX IF NOT EXISTS idx_vocab_chunks_created_at ON vocabulary_chunks_embeddings(created_at DESC)';
  EXECUTE 'CREATE INDEX IF NOT EXISTS idx_vocab_chunks_embedding ON vocabulary_chunks_embeddings USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 64)';
  EXECUTE 'CREATE INDEX IF NOT EXISTS idx_vocab_chunks_user_vocabulary ON vocabulary_chunks_embeddings(user_id, vocabulary_id)';
  EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS idx_vocab_chunks_unique_vocabulary ON vocabulary_chunks_embeddings(vocabulary_id)';
  
  RAISE NOTICE '✅ Index créés';
END $$;

-- ============================================================================
-- Fonction: Recherche sémantique (avec détection automatique du type)
-- ============================================================================
DO $$
DECLARE
  vocab_id_type TEXT;
...
-- Afficher le type de vocabulary_id dans la table créée
SELECT 
  'Type de vocabulary_id' AS info,
  column_name,
  data_type
FROM information_schema.columns
WHERE table_schema = 'public'
