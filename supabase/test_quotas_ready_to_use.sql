-- ============================================================
-- 🧪 SCRIPTS SQL PRÊTS À L'EMPLOI POUR TESTER LES QUOTAS
-- ============================================================
...
-- 1. Créer/mettre à jour le quota
...
BEGIN
  ...
  ON CONFLICT (user_id)
  DO UPDATE SET plan_id = v_free_plan_id, status = 'active', updated_at = now();
END $$;
...
BEGIN
  ...
  ON CONFLICT (user_id)
  DO UPDATE SET plan_id = v_starter_plan_id, status = 'active', updated_at = now();
END $$;
...
BEGIN
  ...
  ON CONFLICT (user_id)
  DO UPDATE SET plan_id = v_pro_plan_id, status = 'active', updated_at = now();
END $$;
...
UPDATE user_quotas 
SET ai_tokens_used = 25000, updated_at = now()
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'redasahraoui1@gmail.com')
  AND period_start = date_trunc('month', now());
...
-- 4. Vérifier avec un nouvel increment (doit bloquer)
SELECT 
...
UPDATE user_quotas 
SET ai_tokens_used = 160000, updated_at = now()
WHERE user_id = (SELECT id FROM auth.users WHERE email = 'redasahraoui1@gmail.com')
  AND period_start = date_trunc('month', now());
