-- ============================================================================
-- Fix du trigger pour indexation automatique des notes
-- ============================================================================
-- Version améliorée qui utilise l'URL Supabase automatiquement
-- ============================================================================

-- Activer l'extension pg_net si elle n'est pas déjà activée
CREATE EXTENSION IF NOT EXISTS pg_net;

-- Fonction améliorée pour appeler l'Edge Function index-note
...
BEGIN
  ...
  IF supabase_url IS NULL OR supabase_url = '' THEN
    -- Option 1: Utiliser une variable d'environnement si disponible
    BEGIN
      supabase_url := current_setting('app.settings.supabase_url', true);
    EXCEPTION WHEN OTHERS THEN
      -- Option 2: Utiliser la valeur par défaut (À REMPLACER)
  ...
  -- Note: Si service_key est NULL, l'appel échouera mais ne bloquera pas l'insertion
  ...
  ELSE
    RAISE WARNING 'Service key non configurée, impossible d''appeler index-note pour la note %', NEW.id;
  END IF;
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'trigger_auto_index_note'
  ) THEN
...
-- Commentaire
COMMENT ON FUNCTION trigger_index_note IS 'Déclenche l''indexation automatique d''une note via l''Edge Function index-note (version améliorée)';
