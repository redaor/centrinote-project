...
export class N8NWebhookService {
  ...
  /**
   * 🎥 Send Daily.co meeting events to N8N
   */
  async sendDailyWebhook(event: string, data: any): Promise<N8NWebhookResponse> {
    const payload: N8NWebhookPayload = {
      source: 'centrinote_daily',
      event,
    ...
    const webhookUrl = import.meta.env.VITE_N8N_DAILY_EVENTS;
    if (!webhookUrl) {
      console.error('❌ VITE_N8N_DAILY_EVENTS not configured');
      return {
        success: false,
        error: 'Daily.co webhook URL not configured'
      };
    }
   ...
   * ✅ PATCH: Ajout timeout + AbortController pour éviter freeze UI
   */
  private async sendDirectWebhook(url: string, payload: N8NWebhookPayload): Promise<N8NWebhookResponse> {
    // 🛡️ TIMEOUT PROTECTION: 8s max (Netlify Functions = 10s max)
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        signal: controller.signal // ✅ PATCH: Ajout signal pour timeout
      });
      ...
      // ✅ PATCH: Gestion spécifique timeout vs erreur réseau
      const isTimeout = error instanceof Error && error.name === 'AbortError';
      console.error(`❌ Error sending webhook to ${url}:`, isTimeout ? 'TIMEOUT (8s)' : error);

      return {
        success: false,
        error: isTimeout
          ? 'Request timeout (8s) - webhook may still process in background'
          : (error instanceof Error ? error.message : 'Unknown error')
      };
    } finally {
      // ✅ PATCH: Toujours cleanup le timeout
  ...
  private async sendWebhook(endpoint: string, payload: N8NWebhookPayload): Promise<N8NWebhookResponse> {
    ...
    try {
      ...
      // ✅ PATCH: Gestion spécifique timeout vs erreur réseau
      const isTimeout = error instanceof Error && error.name === 'AbortError';
      console.error(`❌ Error sending N8N webhook to ${endpoint}:`, isTimeout ? 'TIMEOUT (8s)' : error);
