...
interface WebhookRouterConfig {
  ...
  recordingUrl: string;        // recording (recording seulement)
}
...
export class WebhookRouter {
  ...
  constructor() {
    this.config = {
      ...
      recordingUrl: import.meta.env.VITE_N8N_RECORDING || ''
    };
    ...
    if (!this.config.recordingUrl) {
      console.warn('⚠️ VITE_N8N_RECORDING non configurée');
    }
  }
  ...
  async routeEvent(
    ...
    if (this.EMAIL_EVENTS.has(event) && this.config.recordingUrl) {
      try {
        ...
        console.log(`📧 [ROUTER] Événement recording "${event}" envoyé vers recording`);
      } catch (error) {
        const errorResult = {
        ...
        results.push(errorResult);
        console.error(`❌ [ROUTER] Erreur recording pour "${event}":`, error);
      }
    }
  ...
  /**
   * 📡 Envoie le payload vers un webhook spécifique via le proxy Netlify
   */
  private async sendToWebhook(
    ...
    try {
      // 🚀 PERFORMANCE: Utiliser le proxy Netlify pour éviter CORS
      const proxyPayload = {
        ...payload,
        targetUrl: url
      };
      ...
      if (!result.success) {
        throw new Error(result.error || 'Webhook proxy failed');
      }
      
      console.log(`✅ [ROUTER] Webhook ${type} success via proxy (${responseTime}ms):`, {
        event: payload.event,
        status: response.status,
        workflowId: result.workflowId,
        proxyResponseTime: result.responseTime
      });
