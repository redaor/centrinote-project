...
const NotesManager: React.FC = () => {
  ...
  // Ouvrir le modal de vue (lecture seule)
  ...
  // Composant pour afficher une note en mode grille (nouvelle version UX)
  const NoteCard = ({ note }: { note: Note }) => (
    <div
      ...
      `}
    >
      {/* Icônes d'action rapides (toujours visibles) */}
      <div className="absolute top-2 right-2 flex gap-2 z-10">
        ...
        <div className="relative">
          <button
            onClick={(e) => {
              e.stopPropagation();
              setIsMenuOpen(isMenuOpen === note.id ? null : note.id);
            }}
            className={`p-1 rounded transition-colors ${
              darkMode ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-200 text-gray-500'
            }`}
          ...
          {isMenuOpen === note.id && (
            <div
              ref={menuRef}
              className={`
                absolute right-0 mt-1 w-48 rounded-md shadow-lg z-20
      ...
      <div className="flex items-start justify-between mb-3 pr-28">
        ...
        {note.is_pinned && (
          <Pin className="w-4 h-4 text-blue-500 flex-shrink-0 ml-2" />
        )}
      </div>
  ...
  // Composant pour afficher une note en mode liste (nouvelle version UX)
  ...
  return (
    <div className="p-6">
      ...
      {showViewModal && selectedNote && (
        <>
          <div className="fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => setShowViewModal(false)} />
          <div className={`
            fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50
            ${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-hidden flex flex-col
          `}>
            {/* Header */}
