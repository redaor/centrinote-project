...
import { microTemplates } from '../../data/microTemplates';
...
export function AutomationTemplates({ onSelectTemplate, onCreateCustom }: AutomationTemplatesProps) {
  ...
  const categories = [
    ...
    { value: 'wellbeing', label: 'Wellbeing', icon: Coffee, color: 'green' },
    { value: 'inspiration', label: 'Inspiration', icon: MessageSquare, color: 'amber' },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Micro Templates Section (Beginner Level 🟢) */}
        ...
        <div className="mb-8">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              ...
              <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                Beginner-friendly templates ready in one click
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {microTemplates.map((microTemplate) => {
              const MicroIcon = getTemplateIcon(microTemplate.icon);

              return (
                <motion.div
                  key={microTemplate.id}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  onClick={() => {
                    // Convert micro template to automation template format
                    const converted: any = {
                      id: microTemplate.id,
                      name: microTemplate.name,
                      description: microTemplate.description,
                      category: microTemplate.category,
                      icon: microTemplate.icon,
                      color: microTemplate.color,
                      trigger_schema: microTemplate.trigger,
                      action_schema: microTemplate.action,
                      is_premium: microTemplate.isPremium
                    };
                    onSelectTemplate(converted);
                  }}
                  className={`${
                  ...
                  } border-2 rounded-xl p-4 hover:shadow-xl transition-all duration-200 cursor-pointer group relative overflow-hidden`}
                >
                  {/* Beginner Badge */}
                  <div className="absolute top-2 right-2">
                    <span className="text-xs font-bold px-2 py-1 rounded-full bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300">
                      🟢 EASY
                    </span>
                  </div>

                  <div className="flex items-start space-x-3 mb-3">
                    <div className={`p-3 rounded-lg bg-${microTemplate.color.replace('#', '')}-100 dark:bg-${microTemplate.color.replace('#', '')}-900/30 group-hover:scale-110 transition-transform`}
                      style={{ backgroundColor: `${microTemplate.color}20` }}>
                      <MicroIcon className="w-6 h-6" style={{ color: microTemplate.color }} />
                    </div>
