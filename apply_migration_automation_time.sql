-- =====================================================
-- SCRIPT D'APPLICATION IMMÉDIATE
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✅ Colonne user_local_time ajoutée';
  ELSE
    RAISE NOTICE 'ℹ️ Colonne user_local_time existe déjà';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✅ Colonne user_timezone ajoutée';
  ELSE
    RAISE NOTICE 'ℹ️ Colonne user_timezone existe déjà';
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '✅ Colonne timezone ajoutée à profiles';
  ELSE
    RAISE NOTICE 'ℹ️ Colonne timezone existe déjà dans profiles';
  END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE '📋 Colonnes ajoutées : user_local_time, user_timezone (automations), timezone (profiles)';
  RAISE NOTICE '🔧 Fonction créée : local_time_to_utc()';
END $$;
