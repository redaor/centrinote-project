...
// Version 2.0 avec debug amélioré et gestion d'erreurs
...
exports.handler = async (event, context) => {
  console.log('📥 [CREATE-MEETING-V2] Requête reçue:', event.httpMethod);
  console.log('🔧 [CREATE-MEETING-V2] Context:', context.functionName);
  ...
  try {
    ...
    console.log('🔍 [CREATE-MEETING-V2] Vérification des variables d\'environnement...');
    ...
    console.log('📋 [CREATE-MEETING-V2] Configuration:', {
    ...
    // Validation critique
    if (!DAILY_API_KEY) {
      console.error('❌ [CREATE-MEETING-V2] DAILY_API_KEY manquante');
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ 
    ...
    if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
      console.error('❌ [CREATE-MEETING-V2] Configuration Supabase manquante');
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ 
    ...
    console.log('📝 [CREATE-MEETING-V2] Données reçues:', {
      title,
    ...
    console.log('👥 [CREATE-MEETING-V2] participants.count =', participants.length);
    if (participants.length > 0) {
      console.log('👤 [CREATE-MEETING-V2] Première adresse (sanity check):', participants[0]?.email);
      console.log('👥 [CREATE-MEETING-V2] payload participants sample:', participants[0]);
    }
    ...
    if (!Array.isArray(participants)) {
      console.error('❌ [CREATE-MEETING-V2] participants n\'est pas un array:', typeof participants);
      return {
        statusCode: 400,
        headers,
    ...
    if (invalidParticipants.length > 0) {
      console.error('❌ [CREATE-MEETING-V2] Participants invalides:', invalidParticipants);
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ 
    ...
    console.log('✅ [CREATE-MEETING-V2] Participants validés et sanitisés:', sanitizedParticipants.length);
    ...
    console.log('🏠 [CREATE-MEETING-V2] Création salle Daily.co:', roomName);
    ...
    console.log('ℹ️ [CREATE-MEETING-V2] Webhooks configurés au niveau account Daily Dashboard');
    ...
    console.log('🚀 [CREATE-MEETING-V2] Appel API Daily.co...');
    ...
    console.log('📡 [CREATE-MEETING-V2] Réponse Daily.co status:', dailyResponse.status);
    ...
    try {
      ...
      console.log('✅ [CREATE-MEETING-V2] Salle Daily.co créée:', {
        name: dailyRoom.name,
        url: dailyRoom.url,
      ...
      console.error('❌ [CREATE-MEETING-V2] Erreur parsing Daily.co response:', parseError);
      return {
        statusCode: 500,
        headers,
        body: JSON.stringify({ 
    ...
    console.log('💾 [CREATE-MEETING-V2] Sauvegarde dans Supabase...');
    ...
    console.log('💾 [CREATE-MEETING-V2] Meeting data with participants:', {
      ...meetingData,
    ...
    if (supabaseError) {
      console.error('❌ [CREATE-MEETING-V2] Erreur Supabase:', supabaseError);
      
      // Nettoyer la salle Daily.co en cas d'erreur Supabase
      try {
        console.log('🧹 [CREATE-MEETING-V2] Nettoyage salle Daily.co...');
        await fetch(`${DAILY_BASE_URL}/rooms/${dailyRoom.name}`, {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${DAILY_API_KEY}` }
        });
      } catch (cleanupError) {
        console.error('❌ [CREATE-MEETING-V2] Erreur nettoyage:', cleanupError);
      }
    ...
    console.log('✅ [CREATE-MEETING-V2] Réunion sauvegardée:', meeting.id);
    console.log('👥 [CREATE-MEETING-V2] Participants sauvegardés:', meeting.participants?.length || 0);
    ...
    if (N8N_EVENTS_WEBHOOK) {
      try {
        console.log('🔔 [CREATE-MEETING-V2] Envoi notification n8n...');
        ...
        fetch(N8N_EVENTS_WEBHOOK, {
          ...
          console.warn('⚠️ [CREATE-MEETING-V2] Webhook n8n échoué (non bloquant):', webhookError.message);
        });
        
      } catch (webhookError) {
        console.warn('⚠️ [CREATE-MEETING-V2] Erreur webhook n8n (non bloquant):', webhookError);
      }
    }

    console.log('🎉 [CREATE-MEETING-V2] Réponse complète:', {
    ...
    console.error('❌ [CREATE-MEETING-V2] Erreur générale:', error);
